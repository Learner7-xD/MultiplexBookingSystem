...
import com.multiplex.bean.ShowsBean;
...
public class PublishShowDetailsDao {
	public void publishShowDetailsMethod(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			ShowsBean obj1=new ShowsBean(movieId2,hallId1,slotId1,fromDate1,toDate1);
			st.setInt(1, 0);
			st.setInt(2, obj1.getHallId());
			st.setInt(3,movieId2);
			st.setInt(4, obj1.getSlotId());
			st.setString(5, obj1.getFromDate());
			st.setString(6, obj1.getToDate());
			st.executeUpdate();
			st.close();
			st1.close();
			con.close();
